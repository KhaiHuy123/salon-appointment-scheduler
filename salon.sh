...
serving(){
  ...
  if [[ -z $SERVICE_NAME ]]; then
    echo -e "\nThe service you offer isn't served in our salon"
    return 1
  fi
  ...
  # ask for customer's name if phone number can't be found
  CUSTOMER_NAME=$($PSQL "SELECT name FROM customers WHERE phone='$CUSTOMER_PHONE'")
  if [[ -z $CUSTOMER_NAME ]]; then
    printf "Your phone number doesn't exist in our system, what's your name?\n"
    read CUSTOMER_NAME
